...
import { categories, formatUGX } from "@/data/menu";
import { toggleMenuAvailability, deleteMenuItem } from "./actions";
...
export default async function AdminMenuPage() {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-cocoa/60">
            {items.length} dishes across {grouped.length} categories.
          </p>
        </div>
        <Link
          href="/admin/menu/new"
      ...
      {[...grouped, ...(uncategorised.length ? [{ id: "other", label: "Other", items: uncategorised }] : [])].map(
        (group) => (
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="px-5 py-3">
                        <ToggleAvailabilityButton
                          ...
                          onToggle={toggleMenuAvailability.bind(null, item.id)}
                        />
                      </td>
                      <td className="px-5 py-3 text-right">
                        <div className="flex items-center justify-end gap-4">
                          ...
                          <DeleteButton
                            ...
                            onDelete={deleteMenuItem.bind(null, item.id)}
                          />
                        </div>
                      </td>
                    </tr>
